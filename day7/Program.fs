...
let toAbsolutePaths = // map each dir to its absolute path
    ...
    >> fst
// Solving
let handleCommand (state: State) = function
    | CdRoot -> {state with dirs = ["/"]}
    | CdOut -> { sizes = state.sizes; dirs = state.dirs |> List.tail}
    | Cd dirName -> {state with dirs = dirName::state.dirs}
    | Ls -> state
let handleFile (state: State) (size: int64) =
    { state with sizes = state.dirs |> toAbsolutePaths |> List.fold (add size) state.sizes }
let folder (state: State) = function
    | Command cmd -> cmd |> handleCommand state
    | File size -> handleFile state size
    | _ -> state
let solve =
